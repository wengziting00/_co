...
## Mux16
對於 $\text{Mux16}$ 來說，這個規則必須應用於 $16$ 個位元向量中的每一個對應位元：輸出 $out[0]$：只從 $a[0]$ 和 $b[0]$ 中選一個。輸出 $out[1]$：只從 $a[1]$ 和 $b[1]$ 中選一個。...以此類推輸出 $out[15]$：只從 $a[15]$ 和 $b[15]$ 中選一個。由於所有 $16$ 個選擇都由同一個 $\text{sel}$ 訊號控制，所以我們需要 $16$ 個基礎的 $\text{Mux}$ 晶片並行運作，每個晶片負責一個位元。
